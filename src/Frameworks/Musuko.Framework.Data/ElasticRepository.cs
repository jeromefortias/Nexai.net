namespace Musuko.Core.Data
{
    ...
    public class ElasticRepository : iElasticRepository<Log>
    {
            ...
            try
            {
                ...
                var searchResponse3 = client.Search<Cache>(s => s
                                    ...
                                    .Query(q => q
                                        .MultiMatch(mm => mm
                                            .Query(key)
                                            .Fields(f => f.Field("prompt").Field("Completion"))
        ...
        public List<SymbolicDecoder> SearchSymbolicDecoder(string key)
        {
            List<SymbolicDecoder> cachedObjects;
            key = key.Replace("\n", "");
            int from = 0;
            int size = 25;
            try
            {
                ...
                var client = new ElasticClient(settings);
                //TODO - Extend the search method
                var searchResponse3 = client.Search<SymbolicDecoder>(s => s
                        .Index("symbolicdecoder")
                        .Size(10000)
                ...
                var hitslist = searchResponse3.Hits.ToList();
                cachedObjects = searchResponse3.Documents.ToList<SymbolicDecoder>();
            }
            catch (Exception)
            {
                throw;
        ...
        public List<SymbolicEncoder> SearchSymbolicEncoder(string key)
        {
            List<SymbolicEncoder> cachedObjects;
            key = key.Replace("\n","");
            int from = 0;
            int size = 25;
            try
            {
                ...
                var client = new ElasticClient(settings);
                //TODO - Extend the search method
                var searchResponse3 = client.Search<SymbolicEncoder>(s => s
                        .Index("symbolicencoder")
                        .Size(10000)
                        .Query(q => q.MultiMatch(m => m
                            .Query(key)
                            .Fields(f => f
                ...
                var hitslist = searchResponse3.Hits.ToList();
                cachedObjects = searchResponse3.Documents.ToList<SymbolicEncoder>();
            }
            catch (Exception)
            {
                throw;
