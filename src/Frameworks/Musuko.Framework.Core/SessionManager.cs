namespace Musuko.Framework.Core
{
    ...
    public class SessionManager
    {
        ...
        public string NoteSave(Note n)
        {
            string ret = "";
            try
            {
                Document document = new Document();
                document.Comment = n.Content;
                document.Date = DateTime.Now;
                document.UserName = n.UserName;
                document.MachineName = n.MachineName;
                document.Location = n.Url;
                document.Type = "NOTE";
                document.Blocks = new List<Block>();
                string saveddocumentid = ElasticSearchServer.Save<Document>(document);
                Paragraph p = new Paragraph();
                p.text = n.Content;
                p.Date = DateTime.Now;
                p.UserName = document.UserName;
                p.MachineName = document.MachineName;
                p.Comment = "Paragraph from note " + n.Url;
                p.lenght = n.Content.Length;
                p.includedIns = new List<IncludedIn>();
             
                
                IncludedIn inc = new IncludedIn();
                inc.documentId = saveddocumentid;
                document.Id = saveddocumentid;
                inc.documentLocation = n.Url;
                inc.position = 1;

                p.includedIns = new List<IncludedIn>();
                string savedparagraphid = ElasticSearchServer.Save<Paragraph>(p);
                Block block = new Block();
                block.Id = savedparagraphid;
                block.Position = 1;
                block.Type = "TEXT";
                document.Blocks.Add(block);
                saveddocumentid = ElasticSearchServer.Save<Document>(document);
            }
            catch (Exception ex)
            {
                // Log the error to console
